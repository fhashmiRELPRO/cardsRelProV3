...
namespace RelPro.User.Api.Controllers;
...
public sealed class UserController : ControllerBase
{
    ...
    ///
    /// This endpoint is the **reference implementation** for the team's migration pattern:
    /// `[RequireEntitlement]` → Controller → `IUserService` → `MySqlUserRepository` →
    /// `BaseMySqlRepository.CallProcedureSingleAsync` → `rcp2_read_user` stored proc.
    ///
    /// **Error codes:**
    ...
    public async Task<IActionResult> GetById(int id, CancellationToken ct = default)
    {
        ...
        // Self-lookup is always allowed; looking up any other user requires UserManagement
